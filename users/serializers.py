...
class UserSerializer(BaseSerializer):
    ...
    def validate_email(self, email):
        ...
        if User.objects.filter(email=email).exists():
            raise serializers.ValidationError({"message": "Email này đã tồn tại."})

        return email

    def validate_password(self, password):
        if password is None or not re.fullmatch(r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$",
                                                password.strip()):
    ...
    def validate_identification(self, identification):
        ...
        if len(identification.strip()) > 12:
            raise serializers.ValidationError({"message": "Số CCCD không được vượt quá 12 ký tự."})

        if User.objects.filter(identification=identification).exists():
            raise serializers.ValidationError({"message": "Số CCCD đã tồn tại."})
    ...
    def validate_student_id(self, student_id):
        ...
        if len(student_id) > 10:
            raise serializers.ValidationError({"message": "Mã số sinh viên không được vượt quá 10 ký tự."})
...
class UserUpdateSerializer(serializers.Serializer):
    ...
    def validate_new_password(self, new_password):
        if new_password is None or not re.fullmatch(r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$",
                                                new_password.strip()):
            raise serializers.ValidationError(
                {"message": "Mật khẩu phải chứa ít nhất 8 ký tự, bao gồm chữ hoa, chữ thường, số và ký tự đặc biệt."}
            )
